...
from time_nlu.StringPreHandler import StringPreHandler
from time_nlu.TimePoint import TimePoint
from time_nlu.TimeUnit import TimeUnit
...
class TimeNormalizer:
    ...
    def init(self):
        fpath = os.path.dirname(__file__) + '/resource/regex.pkl'
        try:
    ...
    def parse(self, target, timeBase=arrow.now(), dateFormat="YYYY-MM-DD HH:mm:ss"):
        """
        ...
        :return: 时间单元数组
        """
        logger.info('current time is :%s' % timeBase.format(dateFormat))

        self.isTimeSpan = False
        self.invalidSpan = False
        self.timeSpan = ''
        self.timeInteval = 0  # 时间区间，根据timeSpan进行判断，如果不为0，则数量单位是timeSpan
        self.target = self._filter(target)
        self.timeBase = timeBase.format('YYYY-M-D-H-m-s')
        ...
        dic = {}
        res = self.timeToken
        logger.info('初始化当前的时间基准，时间基准为: %s' % self.timeBase)
        ...
        if self.isTimeSpan:
            ...
            if len(res) == 0:
                ...
                dic['type'] = self.timeSpan

                # 说明是一年
                if self.timeSpan:
                    time = res[0].time
                    if self.timeSpan == 'half-year':
                        startime = time.format(dateFormat)
                        endtime = time.shift(months=6, days=-1).format(dateFormat)
                    else:
                        if self.timeInteval != 0:
                            startime = time.format(dateFormat)
                            if self.timeSpan == 'year':
                                endtime = time.shift(years=self.timeInteval).format(dateFormat)
                            elif self.timeSpan == 'month':
                                endtime = time.shift(months=self.timeInteval).format(dateFormat)
                            else:
                                endtime = time.shift(days=self.timeInteval).format(dateFormat)
                            # endtime = time.ceil(self.timeSpan).format(dateFormat)
                        else:
                            startime = time.format(dateFormat)
                            endtime = time.ceil(self.timeSpan).format(dateFormat)
                    dic['value'] = '%s~%s' % (startime, endtime)
                else:
                    dic['value'] = res[0].time.format(dateFormat)
                dic['time'] = res[0].exp_time
            else:
                dic['type'] = self.timeSpan
                dic['time'] = res[0].exp_time + '至' + res[1].exp_time
                dic['value'] = '%s~%s' % (
                    res[0].time.format(dateFormat), res[1].time.format(dateFormat))
        logger.info('解析出来的时间为')
        logger.info(dic)
        return dic
