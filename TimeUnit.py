...
import arrow
import copy
from time_nlu.TimePoint import TimePoint
from time_nlu.RangeTimeEnum import RangeTimeEnum

try:
    from time_nlu.LunarSolarConverter.LunarSolarConverter import *
except:
    from time_nlu.LunarSolarConverter import *
...
class TimeUnit:
    ...
    def norm_setyear(self):
        ...
        if match is not None:
            ...
            is_year = True
            self.normalizer.timeBase = arrow.get(year, 1, 1).format('YYYY-M-D-H-m-s')
    ...
    def norm_setmonth(self):
        ...
        if match is not None:
            ...
            # 处理倾向于未来时间的情况
            self.preferFuture(1)
            year = self.tp.tunit[0]
            if year < 0:
                year = arrow.get(self.normalizer.timeBase).year

            self.normalizer.timeBase = arrow.get(year, month, 1).format('YYYY-M-D-H-m-s')
            self.normalizer.timeSpan = 'month'
    ...
    def norm_setSpanRelated(self):
        ...
        if match is not None:
            matched = True
            self.normalizer.isTimeSpan = False
            if '前' in self.exp_time:
                hour = -1
            else:
                hour = 1
        ...
        if match is not None:
            ...
            if '前' in self.exp_time:
                ...
                week = 1
            week *= int(match.group())
            cur = cur.shift(weeks=week).floor('week')
            self.normalizer.timeSpan = True
            self.normalizer.timeInteval = abs(week) * 7 - 1
    ...
    def norm_setCurRelated(self):
        ...
        if match is not None:
            flag[2] = True
            self.normalizer.timeSpan = 'half-year'
            cur = arrow.Arrow(cur.year, 1, 1)
        ...
        if match is not None:
            flag[2] = True
            # cur = cur.get()
            cur = arrow.Arrow(cur.year, cur.month, 25)
            month_day = arrow.Arrow(cur.year, cur.month, 1).ceil('month').day
            self.normalizer.timeSpan = 'day'
            self.normalizer.timeInteval = month_day - 25
            # cur = cur.shift(months=-1)
        ...
        if match is not None:
            flag[2] = True
            # cur = cur.get()
            cur = arrow.Arrow(cur.year, cur.month, 10)
            self.normalizer.timeSpan = 'day'
            self.normalizer.timeInteval = 10
            # cur = cur.shift(months=-1)
        ...
        if match is not None:
            flag[2] = True
            # cur = cur.get()
            cur = arrow.Arrow(cur.year, cur.month, 1)
            self.normalizer.timeSpan = 'day'
            self.normalizer.timeInteval = 5
            # cur = cur.shift(months=-1)
